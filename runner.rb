...
options = OpenStruct.new \
  ...
  :bots => Dir.entries("./example_bots").select {|m| m[-4..-1] == ".jar"}, #|| m[0..10] == "planet_wars"
  :players => [1,2],
  :log => "debug.log"
...
OptionParser.new do |parser|
  ...
  parser.on "-b", "--bots [BOTS]", Array, "an optional list of bots to play, defaults to all .jar in ./example_bots" do |bots|
    options.bots = bots.map{|bot| bot[-4..-1] == ".jar" ? bot : bot[0..10] == "planet_wars" ? bot : "#{bot}Bot.jar"}
  end
  parser.on "-t", "--tcp" do
    options.tcp = true
  end
...
options.maps.each do |m|
  options.bots.each do |b|
    ...
    if options.players.include? 1
      `java -jar tools/PlayGame-1.2.jar maps/#{m} 4000 1000 log.txt build/planet_wars \"#{(b[-4..-1] == ".jar" ? "java -jar " : "") + "example_bots/#{b}"}\" 2> #{options.log} #{options.show ? "| java -jar tools/ShowGame.jar" : ""}`
    ...
    if options.players.include? 2
      `java -jar tools/PlayGame-1.2.jar maps/#{m} 1000 1000 log.txt \"#{(b[-4..-1] == ".jar" ? "java -jar " : "") + "example_bots/#{b}"}\" build/planet_wars 2> #{options.log} #{options.show ? "| java -jar tools/ShowGame.jar" : ""}`
      debug = File.read(options.log).split("\n")
      moves = debug.select{|line| (line =~ /Turn \d+/) == 0}.size
      result = debug[-1]
      File.delete options.log
